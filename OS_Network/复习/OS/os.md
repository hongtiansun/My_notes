...
程序：静态的，存放在磁盘中的可执行文件，一系列指令的集合(可装入模块)
进程：动态的，程序的一次执行过程(同一个程序可以对应多个进程)

**进程信息**

进程号：
进程被创建时，操作系统会分配一个唯一的，不重复的进程号，PID
进程递增创建，唯一不重复

进程信息：
- 身份信息：基本的进程描述信息，可以让操作系统区分各个进程
  UID: 进程所属用户
  PID: 进程iD
- 分配资源: 可用于实现操作系统对资源的管理
  分配内存
  使用I/O设备
  使用的文件
- 运行情况: 实现操作系统对进程的控制，调度
  CPU使用时间
  磁盘使用时间
  网络流量使用情况
...
数据结构PCB,保存进程信息
PCB是进程存在的唯一标志。
进程创建时，操作系统创建PCB，进程结束后，操作系统回收PCB。
PCB包含信息：
- 进程描述信息
  PID
  UID
...
举例：Linux task_struct源码

进程实体组成结构：
- PCB: for操作系统
- 程序段: 程序的代码(指令序列) for进程
- 数据段: 运行过程中产生的各种数据(如定义的变量) for进程

**程序运行过程**

基本步骤：
- 写好的程序，经过iso到exe可执行文件，exe中存的就是指令序列
- 运行前把程序存入内存
- 操作系统建立进程 创建PCB
- 程序的指令序列 存入程序段
- CPU从内存中读出指令，执行
...
![](images/APi-2025-03-17-15-31-47.png)

进程与进程实体
- 进程：动态的
- 进程实体：静态的，反映了进程在某一时刻的状态(状态)
  由PCB 程序段 数据段组成

**定义**

进程是进程实体的运行过程，是系统进行资源分配和调度的一个独立单位
所谓调度，就是操作系统决定让这个进程上CPU运行
PCB是进程存在的唯一标志
![](images/APi-2025-03-17-15-33-32.png)
...
![](images/APi-2025-03-17-15-34-52.png)
